...
const long interval = 500;            // Sensor data sending interval
...
void startBlinkingLeft() {
  handleLights(12, 1, 100);
}

void stopBlinkingLeft() {
  handleLights(12, 0, 0);
}

void startBlinkingRight() {
  handleLights(11, 1, 100);
}

void stopBlinkingRight() {
  handleLights(11, 0, 0);
}



void handleLights(int pin, int value, int ldelay) {
  // Lights on, not blinking
  if (value == 1 && (ldelay == 0 || ldelay == NULL)) {
    digitalWrite(pin, HIGH);
    if (pin == 13) {
      pin13_blinking = false;
    }
    if (pin == 12) {
      pin12_blinking = false;
    }
    if (pin == 11) {
      pin11_blinking = false;
    }
  }
  // blinking lights
...
void setup() {
  ...
  for (byte i = 0; i < 6; i++) {
    key.keyByte[i] = 0xFF;  // RFID byte handling
  }
...
void loop() {
  ...
  serialEvent1(); if (stringComplete == true) {
    HandleIncomingJson();
  }
